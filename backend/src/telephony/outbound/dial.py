...
from database import FarmerRepository  # noqa: E402
...
FULL_SIP_URI = f"sip:{LINPHONE_USERNAME}@{LINPHONE_DOMAIN}"
...
def validate_destination(
    ...
    if SIP_ADDRESS.fullmatch(value):
        ...
        username = without_scheme.split("@", 1)[0].strip()
    ...
    raise OutboundCallError(
        "Invalid destination. Use an SIP username such as anushkadubey12."
    )
...
def _required_environment(
    ...
    if not value:
        raise OutboundCallError(f"Missing {name}. Add it to backend/.env.local.")
...
def validate_configuration() -> str:
    ...
    livekit_url = _required_environment("LIVEKIT_URL")
    ...
    _required_environment("LIVEKIT_API_KEY")

    _required_environment("LIVEKIT_API_SECRET")
    ...
    if not trunk_id:
        raise OutboundCallError(
            "Missing LIVEKIT_SIP_OUTBOUND_TRUNK_ID or LIVEKIT_OUTBOUND_TRUNK_ID."
        )
...
async def make_outbound_call(
    ...
    *,
    ...
    # Farmer ID
    farmer_id = (farmer_id or destination).strip()
    ...
    if repository.has_outbound_opt_out(farmer_id):
        raise OutboundCallError(
            "This farmer has opted out. No outbound call was placed."
        )
    ...
    room_name = f"krishimitra-alert-{uuid.uuid4().hex[:12]}"
    ...
    logger.info("KrishiMitra - Outbound Weather Alert Call")
    ...
    livekit_api_key = _required_environment("LIVEKIT_API_KEY")

    livekit_api_secret = _required_environment("LIVEKIT_API_SECRET")
    ...
    async with api.LiveKitAPI(
        ...
        try:
            ...
            logger.info("[STEP 1] Creating LiveKit room...")
            ...
            logger.info("[STEP 2] Dispatching KrishiMitra agent...")

            dispatch_request = api.CreateAgentDispatchRequest(
                agent_name=agent_name,
                room=room_name,
                metadata=json.dumps(OUTBOUND_ALERT_METADATA),
            )

            dispatch = await client.agent_dispatch.create_dispatch(dispatch_request)
            ...
            logger.info("[STEP 3] Creating outbound SIP call...")
            ...
            sip_request = api.CreateSIPParticipantRequest(
                ...
                room_name=room_name,
                participant_identity=(f"learner-{LINPHONE_USERNAME}"),
                participant_name=("KrishiMitra Farmer"),
                display_name=("KrishiMitra AI"),
                participant_attributes={
                    "krishimitra.outbound_alert": "weather",
                    "krishimitra.farmer_id": farmer_id,
                },
            ...
            logger.info("[SUCCESS] Outbound SIP call answered!")
            ...
            logger.info("[KrishiMitra] Farmer connected.")

            logger.info("[KrishiMitra] Weather alert conversation started.")

            logger.info("[KrishiMitra] Agent is now handling the call.")
            ...
            logger.exception("[KrishiMitra] Outbound SIP call failed.")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=("KrishiMitra outbound weather alert call to anushkadubey12.")
    )

    parser.add_argument(
        "--to",
        default=LINPHONE_USERNAME,
        help=("Linphone username. Default: anushkadubey12"),
    )

    parser.add_argument(
        "--farmer-id",
        default=LINPHONE_USERNAME,
        help=("Stable farmer ID used for memory and opt-out checks."),
    )
    ...
    print()
    print("=" * 65)
    print("KrishiMitra outbound call is active")
    print("=" * 65)

    print(f"Target: {FULL_SIP_URI}")

    print(f"LiveKit sip_call_to: {SIP_CALL_TO}")
